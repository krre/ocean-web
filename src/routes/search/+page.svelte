<script lang="ts">
	...
	$effect(() => {
		if (data.type >= 0 && data.text.length >= 0 && data.pageNo >= 1 && untrack(() => isLoaded)) {
	...
	function recordTitle(titleId: number, title: string, id: number, row: number) {
		if (searchType == SearchType.Mandela) {
			return `<a target="_blank" href="${route.Mandela.Id(titleId)}">${title}</a>`;
			...
			return pageUrl(route.Forum.Topic.Id(titleId), title, id, page, true);
		}
	}
</script>
...
<Frame title="Поиск">
	<div class="container">
		...
		<div>
			<WaitButton title="Найти" enabled={isLoaded && searchText.length > 0} sendAction={search} />
		</div>
	</div>
</Frame>
...
{#if records.length}
	<Rectangle padding={false}>
		{#each records as record}
			<div class="record">
				{@html recordTitle(record.title_id, record.title, record.id, record.row)}
