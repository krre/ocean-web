<script lang="ts">
	...
	let mandels = $derived(data.getMandelsResponse ? data.getMandelsResponse.mandels : []);

	let mandelsCount = $derived(data.getMandelsResponse ? data.getMandelsResponse.total_count : 0);

	let users = $derived(data.getUsersResponse ? data.getUsersResponse.users : []);

	let usersCount = $derived(data.getUsersResponse ? data.getUsersResponse.user_count : 0);
...
<Frame title="Рейтинг">
	<label>
		<input type="radio" bind:group={type} value={types.RatingType.Mandels} />
		Манделы
	</label>
	...
	{#if type === types.RatingType.Mandels}
		<MandelaRating bind:vote {mandels} pageNo={data.pageNo} pageLimit={local.PageLimit} />
	{:else}
