		...
		{#if topicPoll}
			...
			{:else}
				{#each topicPoll as answer}
					{#if pollSelectionType === types.ForumPollAnswerSelection.One}
						<label class="vote">
							<input type="radio" bind:group={oneVote} value={answer.id} />
							{answer.answer}
						</label>
					{:else}
						<label class="vote">
							<input type="checkbox" bind:group={severalVote} value={answer.id} />
							{answer.answer}
						</label>
					{/if}
				{/each}
				...
				<button
					onclick={castVote}
					disabled={pollSelectionType == types.ForumPollAnswerSelection.One
						? oneVote < 0
...
{#if !$userSession.isAnonym || isAnonymAllowed()}
	<MessageEditor bind:this={messageEditorRef} bind:message={post} sendAction={() => append()} />
{/if}
