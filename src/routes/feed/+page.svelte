<script lang="ts">
	...
	function titleRoute(id: number, type: string): string {
		if (type == api.Feed.TitleType.Mandela || type == api.Feed.TitleType.Comment) {
	...
	function dateRoute(
		...
		if (type == api.Feed.TitleType.Mandela || type == api.Feed.TitleType.Topic) {
...
<Pagination count={totalCount} limit={PageLimit} offset={pageNo} baseRoute={route.Feed.Root} />
