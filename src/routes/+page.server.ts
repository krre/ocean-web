...
async function loadActivity(): Promise<[ActivityMessage[], ActivityMessage[]]> {
	...
	result.topics.forEach((item: api.Activity.Topic) => {
		const topic: ActivityMessage = {
			...
			pageNo: Math.ceil(item.post_count / consts.Forum.Post.PageLimit),
	...
	result.comments.forEach((item: api.Activity.Comment) => {
		const comment: ActivityMessage = {
			...
			pageNo: Math.ceil(item.comment_count / consts.Mandela.Comment.PageLimit),
			date: item.create_ts,
			userName: item.user_name,
			userId: item.user_id,
