...
{#if comments.length}
		...
		{#each comments as comment, i}
			<div class="post" id={comment.id.toString()}>
				<PostTitle
					...
					likeSelection={comment.user_id === $userSession.id || $userSession.isAnonym
					...
					editable={comment.user_id === $userSession.id || $userSession.isAdmin}
					removable={$userSession.id === comment.user_id || $userSession.isAdmin}
	...
	<Pagination count={commentCount} limit={pageLimit} offset={pageNo} {baseRoute} />
{/if}

{#if !$userSession.isAnonym || isAnonymAllowed()}
	<MessageEditor bind:message bind:this={messageEditorRef} sendAction={() => append()} />
{/if}
