<script lang="ts">
	...
	$effect(() => {
		editable =
			$userSession.isAdmin || (!$userSession.isAnonym && $userSession.id === topic.user_id);
	});
...
<div class="topic">
	...
	{#if editable}
		<span>
			<button onclick={editTopic} title="Редактировать"><i class="fas fa-edit"></i></button>
			<button onclick={removeTopic} title="Удалить"><i class="fas fa-trash-alt"></i></button>
		</span>
	{/if}
	<div class="info">
		{@html userUrl(topic.user_name, topic.user_id)} · {formatDateTime(topic.create_ts)}
	</div>
	<div class="info">
