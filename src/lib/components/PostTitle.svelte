<script lang="ts">
	...
	function usersForAction(users: api.Like.GetUsers.Response[], action: LikeAction): string {
		let result = '';
...
<div class="title">
	{@html userUrl(userName, userId)} · {@html dateUrl(baseUrl, date, id, pageNo)}
	...
	<span>
		{#if likable}
			...
			{:else}
				<button
					...
					aria-label="Нравится"
					onclick={() => {
						likeSelection == LikeSelection.Like ? like(LikeAction.Unlike) : {};
					}}
				>
					<i class="far fa-thumbs-up {likeSelection == LikeSelection.Like ? 'like up' : ''}"></i>
				</button>
			{/if}
			...
			{#if likeSelection == LikeSelection.None}
				<button
					...
					title="Не нравится"><i class="far fa-thumbs-down"></i></button
				>
			{:else}
				&nbsp;
				<button
					...
					onclick={() => {
						likeSelection == LikeSelection.Dislike ? like(LikeAction.Unlike) : {};
					}}
				>
					<i class="far fa-thumbs-down {likeSelection == LikeSelection.Dislike ? 'like down' : ''}"
					></i>
				</button>
			{/if}
			...
			{#if likeQuestion && (likeCount || dislikeCount)}
				<button
					...
					title="Пользователи, поставившие оценку"><i class="fas fa-question"></i></button
				>
			{/if}
		...
		{#if editable}
			<button onclick={edit} title="Редактировать"><i class="fas fa-edit"></i></button>
		{/if}

		{#if removable}
			<button onclick={remove} title="Удалить"><i class="fas fa-trash-alt"></i></button>
		{/if}

		{#if replyable}
			<button onclick={reply} title="Ответить"><i class="fas fa-reply"></i></button>
		{/if}
	</span>
</div>
