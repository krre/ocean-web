<script lang="ts">
	...
	export function appendReply(userName: string, text: string) {
		...
		const message = selection && text.includes(selection) ? selection : text;
	...
	const replaceSelection = (handler: (str: string) => string) => {
		...
		post = post.substring(0, start) + result + post.substring(end, post.length);
	};
	...
	function onOkLink(link: string) {
		if (areaRef.selectionStart != areaRef.selectionEnd) {
			...
			post = insertText(post, areaRef.selectionStart, `⁅url⁆${link}⁅/url⁆`);
		}
	}
	...
	function onOkSpoiler(description: string) {
		if (areaRef.selectionStart != areaRef.selectionEnd) {
			replaceSelection((str: string) => `⁅spoiler="${description}"⁆${str}⁅/spoiler⁆`);
		} else {
			post = insertText(
				post,
...
<div class="container">
	<div class="toolbar">
		<div>
			<button onclick={appendBold} title="Жирный"><i class="fas fa-bold"></i></button>

			<button onclick={appendItalic} title="Курсив"><i class="fas fa-italic"></i></button>
			...
			<button onclick={() => openDialog(LinkDialog, { onOk: onOkLink })} title="Ссылка"
			...
			<button onclick={() => openDialog(SpoilerDialog, { onOk: onOkSpoiler })} title="Спойлер"
				><i class="fas fa-eye-slash"></i></button
			>

			<button onclick={() => (isSmilePanelActive = !isSmilePanelActive)} title="Смайл"
	...
	{#if !isPreview}
		<textarea class="area" rows="10" bind:value={post} bind:this={areaRef}></textarea>
	{:else}
		<div class="preview">
