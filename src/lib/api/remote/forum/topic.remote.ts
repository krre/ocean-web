...
export const create = command(
	v.object({
		...
		name: v.string(),
		type: v.number(),
		poll_answers: v.optional(v.array(v.string())),
		poll_answer_selection: v.optional(v.number())
	}),
	async ({ section_id, name, type, poll_answers, poll_answer_selection }) => {
		return await api.Forum.Topic.Create.exec({
			section_id,
			name,
			type,
			poll_answers,
...
export const vote = command(
	v.object({ id: v.number(), votes: v.array(v.number()) }),
	async ({ id, votes }) => {
