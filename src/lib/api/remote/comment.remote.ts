...
export const create = command(
	v.object({ mandela_id: v.number(), message: v.string() }),
	async ({ mandela_id, message }) => {
		await api.Comment.Create.exec({ mandela_id, message });
	}
);

export const getAll = command(
	v.object({ mandelaId: v.number(), pageNo: v.number() }),
	async ({ mandelaId, pageNo }) => {
		return await comments.loadComments(mandelaId, pageNo);
	}
);

export const update = command(
	v.object({ id: v.number(), message: v.string() }),
	async ({ id, message }) => {
