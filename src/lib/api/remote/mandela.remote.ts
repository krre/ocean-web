...
export const create = command(
	...
	async ({ title_mode, title, what, before, after, description, categories }) => {
		return await api.Mandela.Create.exec({
			title_mode,
			title,
			what,
			before,
...
export const updateTrash = command(
	v.object({
		id: v.number(),
		trash: v.boolean(),
		automatic_trash: v.boolean()
	}),
...
export const vote = command(
	v.object({ id: v.number(), vote: v.number() }),
	async ({ id, vote }) => {
