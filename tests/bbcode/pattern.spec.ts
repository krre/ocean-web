...
describe('BBCode pattern', () => {
	describe('txt', () => {
		test('success', () => {
			expect(txt('abc').exec('abc', 0)).toStrictEqual({
				res: 'abc',
				end: 3
			});
		});
	...
	describe('rgx', () => {
		test('success', () => {
			expect(rgx(/\d+/).exec('123', 0)).toStrictEqual({
				res: '123',
				end: 3
			});
		});
	...
	describe('opt', () => {
		test('success', () => {
			expect(opt(txt('abc')).exec('abc', 0)).toStrictEqual({
				res: 'abc',
				end: 3
			});
		});
		...
		test('failure', () => {
			expect(opt(txt('abc')).exec('123', 0)).toStrictEqual({
				res: undefined,
				end: 0
			});
		});
	...
	describe('seq', () => {
		...
		test('success', () => {
			expect(p.exec('abcdef', 0)).toStrictEqual({
				res: ['abc', 'def'],
				end: 6
			});
		});
	...
	describe('rep', () => {
		...
		test('success 1', () => {
			expect(p.exec('1,23,456', 0)).toStrictEqual({
				res: ['1', '23', '456'],
				end: 8
			});
		});
